	...
	<section class="p-6 bg-[#F2F2F2]">
		...
		<div class="rounded-lg">
			<div class="grid grid-cols-2 gap-4">
				<div v-for="(meal, index) in hotMeals" :key="index"
					...
					<div class="relative">
						<img class="w-[150px] h-[150px] rounded-[20px] object-contain" :src="meal.image"
							alt="">
...
// Function to decrease order count (ensuring it doesn't go below 0)
const decreaseOrder = (meal) => {
  const item = order.value.find(item => meal in item)
  item[meal]-=1
  if(item[meal] == 0){
	order.value.splice(order.value.indexOf(item[meal]))
  }
};
</script>
